...
namespace Michsky.MUIP
{
    ...
    public class ContextMenuContent : MonoBehaviour, IPointerClickHandler, IPointerEnterHandler
    {
        // Resources
        ...
        public void ProcessContent()
        {
            ...
            for (int i = 0; i < contexItems.Count; ++i)
            {
                ...
                if (nulLVariable == false)
                {
                    ...
                    else if (contextManager.contextSubMenu != null && contexItems[i].subMenuItems.Count != 0)
                    {
                        ...
                        if (fixedMenu)
                        {
                            subMenu.shadow.SetActive(false);
                        }
                        else
                        {
                            subMenu.shadow.SetActive(true);
                        }
            ...
            else if (menuSpawnPos != null)
            {
                contextManager.fixedMenu = true;
                contextManager.SetContextMenuPosition(menuSpawnPos.position);
            }
        ...
        {
            if (contextManager.isOn == true && fixedMenu == false) { contextAnimator.Play("Menu Out"); contextManager.isOn = false; }
            else if (eventData.button == PointerEventData.InputButton.Right && contextManager.isOn == false && fixedMenu == false) { contextManager.fixedMenu = false; ProcessContent(); }
        }
