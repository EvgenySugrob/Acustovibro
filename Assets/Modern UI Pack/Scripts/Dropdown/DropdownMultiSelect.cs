...
namespace Michsky.MUIP
{
    public class DropdownMultiSelect : MonoBehaviour, IPointerExitHandler, IPointerClickHandler
    {
        ...
        public GameObject borders; //Extension of script for another visualizing of dropdown. Used optionaly
        ...
        void OnEnable()
        {
            ...
            if (borders != null)
            {
                mainObject = GetComponent<Button>().targetGraphic.gameObject;
                bordersRect = borders.GetComponent<RectTransform>();
                fillRect = borders.transform.GetChild(0).GetComponent<RectTransform>();
            }
        ...
        void ProcessModularAnimation()
        {
            if (isOn == true)
            {
                ...
                if (borders != null)
                {
                    fillRect.GetComponent<Image>().color =
                        new Color(fillRect.GetComponent<Image>().color.r,
                        fillRect.GetComponent<Image>().color.g,
                        fillRect.GetComponent<Image>().color.b,
                        Mathf.Clamp01(fillRect.GetComponent<Image>().color.a + Time.unscaledDeltaTime * transitionSmoothness * 1.5f));

                    mainObject.GetComponent<Image>().enabled = false;
                }
                ...
                { 
                    ...
                    if (borders != null)
                    {
                        float posY = mainObject.GetComponent<RectTransform>().localPosition.y;

                        if (fillRect.anchorMin.y == 1)
                        {
                            bordersRect.offsetMax = new Vector2(bordersRect.offsetMax.x, -posY * 2);
                            //fillRect.sizeDelta = new Vector2(fillRect.sizeDelta.x, -posY * 2);
                        }
                        else if (fillRect.anchorMin.y == 0)
                        {
                            bordersRect.offsetMin = new Vector2(bordersRect.offsetMin.x, posY * 2);
                            //fillRect.sizeDelta = new Vector2(fillRect.sizeDelta.x, -posY * 2); 
                        }
                    }
                }
            ...
            else
            {
                ...
                if (borders != null)
                {
                    ...
                    mainObject.GetComponent<Image>().enabled = true;
                }

                //With "this.enabled = false" dropdown work is incorrectly and disabled after first animation
