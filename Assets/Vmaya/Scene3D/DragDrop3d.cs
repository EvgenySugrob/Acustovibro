...
namespace Vmaya.Scene3D
{
    public class DragDrop3d : HitMouse, IHandle, IDragControl
    {
        ...
        {
            return !VKeyboard.GetKey(Key.LeftAlt) && !EventSystem.current.IsPointerOverGameObject() && !freeze;
        }

        public override void doOver()
        {
            if (!VKeyboard.GetKey(Key.LeftAlt) && !EventSystem.current.IsPointerOverGameObject()) base.doOver();
        }
        ...
        override protected void doMouseDrag()
        {
            if (drag > 0)
            {
                ...
                if ((drag == 1) && (delta.magnitude > hitDetector.instance.DRAGSTICKING))
                {
                    doBeginDrag();
                    drag = 2;
                    _currentDrag = this;
